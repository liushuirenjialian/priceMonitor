  ...
  <div class="dashboard-container">
    <el-form :model="peopleform" :inline="true" label-width="118px" ref="peopleform">
      <el-row>
        <el-col :span="12">
          <div>
            <el-form-item label="监测点类型名称" class="fourLabel">
              <el-input placeholder="请输入" v-model="peopleform.monitorName" clearable>
              </el-input>
            </el-form-item>
          </div>
        </el-col>
    ...
    <el-row>
      <el-table tooltip-effect="dark" style="width: 100%;margin-top:10px;" align="center" border stripe highlight-current-rowv-loading="tableLoading" :data="tableData">
        ...
        <el-table-column align="center" :show-overflow-tooltip="true" label="监测点类型名称" prop="monitorName">
        </el-table-column>
        <el-table-column align="center" :show-overflow-tooltip="true" label="监测点类型级别" prop="monitorLevel">
        </el-table-column>
        <el-table-column align="center" :show-overflow-tooltip="true" label="父监测点类型" prop="companyHierarchy">
        </el-table-column>
...
import { monitorpage } from '@/api/priceArea/index'
export default {
  name: 'Daily',
  data() {
    return {
  ...
  methods: {
    query(obj, dataObj) {
      monitorpage(obj, dataObj).then(res => {
        this.tableData = res.data.resultList
        this.limit.total = res.data.total
      })
    },
    see(row) {
      this.$router.push({ path: '/monitorArea/typeDetail', query: { id: row.id, status: 'detail' } })
    },
    ...
    addBtn() {
      this.$router.push({ path: '/monitorArea/typeDetail' })
    }
  }
}
</script>
